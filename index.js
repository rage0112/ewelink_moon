            ...
            this.webClient.get('/api/user/device', function(err, res, body) {
                ...
                function checkIfDeviceIsAlreadyConfigured(value, deviceId, map) {
                    ...
                    if (platform.accessories.has(deviceId)) {
                        ...
                        switch(deviceToAdd.productModel) {
                            case 'T1 2C' :
                                switchesAmount = 2;
                                break;
                            case 'T1 3C' :
                ...
                platform.wsc.onmessage = function(message) {
                    ...
                    if (json.hasOwnProperty("action")) {
                        ...
                        if (json.action === 'update') {
							...
							else if (json.hasOwnProperty("params") && json.params.hasOwnProperty("switches")) {
								for(let i=0; i!==3; i++) {
									platform.updatePowerStateCharacteristic(json.deviceid + 'CH' + (i+1), json.params.switches[i].switch, null, i);
								}
							}
                        }
...
eWeLink.prototype.updatePowerStateCharacteristic = function(deviceId, state, device = null, channel = null) {
    ...
    platform.log("Updating recorded Characteristic.On for [%s] to [%s].", accessory.displayName, isOn);

    //accessory.getService(Service.Switch)
    //    .setCharacteristic(Characteristic.On, isOn);
	accessory.getService(Service.Switch).getCharacteristic(Characteristic.On).updateValue(isOn);	
		
	if(accessory.context.switches > 1) {
		deviceId = deviceId.replace("CH" + accessory.context.channel, "");
		platform.devicesFromApi.get(deviceId).params.switches[accessory.context.channel - 1].switch = isOn ? 'on' : 'off';
	}
	else {
		platform.devicesFromApi.get(deviceId).params.switch = isOn ? 'on' : 'off';
	}
};
...
eWeLink.prototype.getPowerState = function(accessory, callback) {
    ...
    this.webClient.get('/api/user/device', function(err, res, body) {
        ...
        if (err){
            platform.log("%s An error was encountered while requesting a list of devices while interrogating power status.",accessory.displayName);
			platform.getPowerState(accessory, callback);
            return;
        } else if (!body || body.hasOwnProperty('error')) {
...
eWeLink.prototype.setPowerState = function(accessory, isOn, callback) {
    ...
    if(accessory.context.switches > 1) {
		...
		platform.devicesFromApi.get(deviceId).params.switch = targetState;
    }
